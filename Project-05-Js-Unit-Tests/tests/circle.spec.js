...
*/
const newCircle = Object.assign({}, circle(3));
newCircle.area = Number(parseFloat(newCircle.area).toPrecision(4));
...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contedos os valores esperados', () => {
    ...
    expect(newCircle.area).toEqual(28.26);
    // Teste que a função retorna, num objeto, os dados corretos de um círculo. de raio 3.
    expect(newCircle).toMatchObject({ radius: 3, area: 28.26, circumference: 18.84 });
  });
});
